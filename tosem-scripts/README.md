...
If you are running the analysis on new raw data, this should be your `WORKDIR` 
(from `tools/captain/captainrc`): but note that some of our scripts
(notably, those extracting instrumentation time) assume that this contains results for only
one fuzzer, and will not work correctly if the `WORKDIR` merges results for 
multiple fuzzers. 
