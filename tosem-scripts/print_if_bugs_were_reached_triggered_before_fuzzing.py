...
def happened_before_fuzzing_started(bug_time, pure_fuzz_time, reached_or_triggered= "reached"):
    """
    ...
    If that interval is wholly in loading the queue, say 'seed loading'
    If that interval is wholly while fuzzing, say 'true fuzzing'
    Else, say 'maybe', and the time before the bug would have had to happen
    for it to be 'seed loading'
    ...
    'maybe before (600s of fuzzing)'
    >>> happened_before_fuzzing_started(5, 599)
    'maybe before (if happened in 1s rather than 5s)'
    >>> happened_before_fuzzing_started(5, 596)
    'maybe before (if happened in 4s rather than 5s)'
    ...
    'maybe before (if happened in 22s rather than 25s)'
    >>> happened_before_fuzzing_started(10, 596, "reached")
    'true fuzzing'
    >>> happened_before_fuzzing_started(10, 596, "triggered")
    'maybe before (if happened in 4s rather than 10s)'
    """
    when_finished_queue = timeout - pure_fuzz_time
    if reached_or_triggered != "reached" and reached_or_triggered != "triggered":
        print("Incorrect argument: ", reached_or_triggered, file=sys.stderr)
        return
    poll_lag = poll if reached_or_triggered == "reached" else 2*poll
    if bug_time < when_finished_queue:
        return 'seed loading'
    elif bug_time - poll_lag < when_finished_queue:
        return f'maybe before (if happened in {when_finished_queue}s rather than {bug_time}s)'
    elif when_finished_queue == 0 and bug_time < poll_lag + 1:
        return f'maybe before ({timeout}s of fuzzing)'
    else:
        return 'true fuzzing'
    ...
    for benchmark in total_reached[fuzzer]:
        ...
        for i, reached_time in enumerate(reached_times):
            ...
            if true_fuzzing_time == 'NaN':
                ...
                continue
            res = happened_before_fuzzing_started(int(reached_time), int(true_fuzzing_time))
